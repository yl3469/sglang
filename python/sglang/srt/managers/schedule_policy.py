...
# ---------------------------------------------------------------------------
# UniBoost (ICML'26 "beyond prediction"): tail-aware prefill admission
# boosting. Selected via --schedule-policy uniboost; tuned via env vars.
# ---------------------------------------------------------------------------
UNIBOOST_GAMMA = float(os.environ.get("SGLANG_UNIBOOST_GAMMA", "10.0"))
UNIBOOST_ALPHA_US_PER_TOKEN = float(
    os.environ.get("SGLANG_UNIBOOST_ALPHA_US_PER_TOKEN", "64.0")
)
UNIBOOST_ALLOW_TP = get_bool_env_var("SGLANG_UNIBOOST_ALLOW_TP")
UNIBOOST_GAMMA_ADA = get_bool_env_var("SGLANG_UNIBOOST_GAMMA_ADA")
UNIBOOST_GAMMA_MIN = float(os.environ.get("SGLANG_UNIBOOST_GAMMA_MIN", "1.0"))
UNIBOOST_GAMMA_MAX = float(os.environ.get("SGLANG_UNIBOOST_GAMMA_MAX", "200.0"))
UNIBOOST_GAMMA_ADA_INTERVAL_S = float(
    os.environ.get("SGLANG_UNIBOOST_GAMMA_ADA_INTERVAL_S", "5.0")
)
UNIBOOST_GAMMA_ADA_WINDOW = int(
    os.environ.get("SGLANG_UNIBOOST_GAMMA_ADA_WINDOW", "2000")
)
UNIBOOST_GAMMA_ADA_MIN_SAMPLES = int(
    os.environ.get("SGLANG_UNIBOOST_GAMMA_ADA_MIN_SAMPLES", "200")
)
UNIBOOST_GAMMA_ADA_BETA = float(
    os.environ.get("SGLANG_UNIBOOST_GAMMA_ADA_BETA", "0.3")
)
UNIBOOST_TAIL_SENSITIVITY = float(
    os.environ.get("SGLANG_UNIBOOST_TAIL_SENSITIVITY", "4.0")
)
...
class SchedulePolicy:
    ...
    def __init__(
        ...
        self.uniboost_alpha_s_per_token = UNIBOOST_ALPHA_US_PER_TOKEN / 1e6
    ...
    def calc_priority(
        ...
        if isinstance(policy, CacheAwarePolicy):
            if policy == CacheAwarePolicy.UNIBOOST:
                ...
                if not getattr(self.tree_cache, "disable", True):
    ...
    def _init_uniboost(self) -> None:
        ...
        tp_size = 1
        try:
            tp_size = int(get_global_server_args().tp_size)
        except Exception:
            pass
        if tp_size > 1 and not UNIBOOST_ALLOW_TP:
            logger.warning(
        ...
        if UNIBOOST_GAMMA_ADA:
            self.uniboost_gamma_controller = GammaAdaController(
                ...
                beta=UNIBOOST_GAMMA_ADA_BETA,
                tail_sensitivity=UNIBOOST_TAIL_SENSITIVITY,
                interval_s=UNIBOOST_GAMMA_ADA_INTERVAL_S,
                window=UNIBOOST_GAMMA_ADA_WINDOW,
                min_samples=UNIBOOST_GAMMA_ADA_MIN_SAMPLES,
            )
    ...
    def _uniboost_priority(self, req: Req, now: float) -> float:
        ...
        arrival = getattr(req.time_stats, "wait_queue_entry_time", 0.0) or now
    ...
    def uniboost_record_finished(self, reqs: List[Req]) -> None:
        ...
        for req in reqs:
            if not req.finished():
                continue
            if getattr(req, "_uniboost_latency_recorded", False):
                continue
            req._uniboost_latency_recorded = True
            if isinstance(req.finished_reason, FINISH_ABORT):
                continue
            arrival = getattr(req.time_stats, "wait_queue_entry_time", 0.0)
            if not arrival or arrival <= 0.0:
                continue
